	...
	<fieldset class="fieldset-login_required">
		...
		<div class="field account-sign-in">
			...
			<?php if ( $registration_enabled ) : ?>
				...
				<?php printf( esc_html__( 'If you don\'t have an account you can create one below by entering your email address/username.', 'wp-job-manager' ), $account_required ? '' : esc_html__('optionally', 'wp-job-manager') . ' account-signin.php'); ?>
	...
	if ( ! empty( $registration_fields ) ) {
		foreach ( $registration_fields as $key => $field ) {
			...
			<fieldset class="fieldset-<?php echo esc_attr( $key ); ?>">
				<label
					for="<?php echo esc_attr( $key ); ?>"><?php echo ' <small>' . __('(optional)', 'wp-job-manager'); ?></label>
